...
function processMatchUpdate(globals, match, updatePath) {
  ...
  if (updatePath && updatePath[0]) {
    ...
    if (updatePath.includes('score'))
      ...
      processMatchStartedUpdate(globals, match);
    else if (updatePath.includes('finished'))
      processMatchFinishedUpdate(globals, match);
  }
}

// Processes a kick off update for the given match
function processMatchStartedUpdate(globals, match) {
  console.log('Processing match started update for match: ', match.id, match.started);
  if (match.started) {
    let title = `🕒 Kick off: ${match.homeTeam.names.displayName} vs ${match.awayTeam.names.displayName}`;
    let subtitle = '';

    // If the match is on TV, add the channel name to the title
    const tvChannelName = _.get(match, 'tvInfo.channelInfo.fullName');
    if (tvChannelName) {
      subtitle += `🖥️ Watch on ${tvChannelName}`; 
    }
...
// Processes a full time update for the given match
function processMatchFinishedUpdate(globals, match) {
  console.log('Processing match finished update for match: ', match.id, match.finished);
  const ttl = _.get(globals, 'config.notifications.matchUpdateTtlSeconds', 86400);

  const homeTeamName = _.get(match, 'homeTeam.names.displayName');
  const awayTeamName = _.get(match, 'awayTeam.names.displayName');

  const homePenaltyScore = _.get(match, 'homePenaltyScore');
  const awayPenaltyScore = _.get(match, 'awayPenaltyScore');

  let title = `🕒 Full time: ${homeTeamName}  ${match.homeTeam.score} - ${match.awayTeam.score}  ${awayTeamName}`;
  let subtitle = '';

  // If there's an aggregate score, add it to the title
  const aggregateScore = _.get(match, 'aggregateScore');
  if (aggregateScore && aggregateScore.length > 0) {
    subtitle += utils.getAggregateScoreMessage(homeTeamName, awayTeamName, aggregateScore, homePenaltyScore, awayPenaltyScore)
  }

  // If the match ended in a penalty shootout, add the score of the winning team to the title
  if (homePenaltyScore && awayPenaltyScore && homePenaltyScore >= 0 && awayPenaltyScore >= 0) {
    if (homePenaltyScore > awayPenaltyScore) {
      subtitle += ` (${homeTeamName} win ${homePenaltyScore} - ${awayPenaltyScore} on penalties)`;
    }
    else {
      subtitle += ` (${awayTeamName} win ${awayPenaltyScore} - ${homePenaltyScore} on penalties)`;
    }
  }
...
function processScoreUpdate(globals, match, updatePath) {
  ...
  const message = {
    ...
    subtitle: `${scoringTeamName === match.homeTeam.names.displayName ? '⚽️ ' : ''}${match.homeTeam.names.displayName}  ${match.homeTeam.score} - ${match.awayTeam.score}  ${match.awayTeam.names.displayName} ${scoringTeamName === match.awayTeam.names.displayName ? '⚽️' : ''} ${timeLabel}`,
    body: getCompetitionLabel(match),
    type: 'score_updates'
  };
  send(globals, match, message, ttl);
...
// If it's half time (HT), send a notification
...
async function send(globals, match, message, ttl) {
  ...
  for (const user of await users.getAll()) {
    ...
    // If the user is interested in receiving notifications for the match,
    ...
    if (match.interestedUsers.includes(user.id)) {
      ...
      if (token) {
        ...
        const preferences = _.get(globals.data.cache.userPreferences, user.id);
