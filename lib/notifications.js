...
function processMatchFinishedUpdate(globals, match) {
  ...
  if (aggregateScore && aggregateScore.length > 0) {
    title += utils.getAggregateScoreMessage(homeTeamName, awayTeamName, aggregateScore, homePenaltyScore, awayPenaltyScore)
  }
  ...
  if (homePenaltyScore && awayPenaltyScore && homePenaltyScore >= 0 && awayPenaltyScore >= 0) {
    if (homePenaltyScore > awayPenaltyScore) {
      title += ` (${homeTeamName} win ${homePenaltyScore} - ${awayPenaltyScore} on penalties)`;
    }
    else {
      title += ` (${awayTeamName} win ${awayPenaltyScore} - ${homePenaltyScore} on penalties)`;
    }
  }
