...
function processMatchStartedUpdate(globals, match) {
  ...
  if (match.started) {
    ...
    const message = {
      title: `🕒 Kick off: ${match.homeTeam.names.displayName} vs ${match.awayTeam.names.displayName}`,
...
function processMatchFinishedUpdate(globals, match) {
  ...
  const ttl = _.get(globals, 'config.notifications.matchUpdateTtlSeconds', 86400);
  const message = {
    title: `🕒 Match finished: ${match.homeTeam.names.displayName}  ${match.homeTeam.score} - ${match.awayTeam.score}  ${match.awayTeam.names.displayName}`,
    body: getCompetitionLabel(match),
    type: 'full_time'
  };
  send(globals, match, message, ttl);
