...
async function parseMatchesForDate(globals, date) {
    ...
    const url = `${globals.config.urls.matchesApi.replace('{YYYYMMDD}', matchesApiDate)}`;
...
function getMatchTimeLabel(match) {
    ...
    // If the match is finished, return "FT" in all circumstances except for "AET"
...
function getMatchInfo(globals, competitionSubHeading, competitionId, isInternationalCompetition, match, date) {
    ...
    // Status messages
    _.set(matchInfo, 'statusMessages', getMatchStatusMessages(match, homeTeamNames.displayName, awayTeamNames.displayName));
...
function getMatchStatusMessages(match, homeTeamName, awayTeamName) {
    ...
    if (matchStatus && matchStatus !== 'Pen') {
        statusMessages.push(_.get(match, matchStatus));
    }
    ...
    if (!match.status.finished && liveTimeShort !== 'Pen' && (_.get(match, 'status.halfs.firstExtraHalfStarted') || _.get(match, 'status.halfs.secondExtraHalfStarted'))) {
    ...
    // Otherwise, if penalties are in progress, add a message to that effect
        ...
        statusMessages.push('Penalties in progress');
    }
    ...
    // Only display where there is no penalty score
    const aggregateScoreInfo = _.get(match, 'status.aggregatedStr');
    if (!homePenaltyScore && !awayPenaltyScore && aggregateScoreInfo) {
        statusMessages.push(`Aggregate score: ${aggregateScoreInfo}`);
    }
...
function getPenaltiesResultStatusMessage(homeTeamName, awayTeamName, homePenaltyScore, awayPenaltyScore) {
    if (homePenaltyScore > awayPenaltyScore) {
        return `${homeTeamName} won ${homePenaltyScore} - ${awayPenaltyScore} on penalties`;
    }
    else if (homePenaltyScore < awayPenaltyScore) {
        return `${awayTeamName} won ${awayPenaltyScore} - ${homePenaltyScore} on penalties`;
    }
    else {
...
async function getResults(globals, deviceId, limit) {
    ...
    for (const match of matches) {
        const isMatchFinished = _.get(match, 'status.finished', false);
        if (moment(match.date, 'YYYY-MM-DD').isBefore(moment(), 'day') || isMatchFinished)
            results.push(match);
    }
...
async function getMatchesProcessed(globals, matches, deviceId, dateOrder, limit = 0) {
    ...
    for (const match of matches) {
        if ((deviceId && match.interestedUsers && match.interestedUsers.includes(deviceId)) || globals.config.leagues.default.includes(match.competition__id))
