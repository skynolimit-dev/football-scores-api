...
function getCompetitionsOfInterestForUser(globals, deviceId) {
    ...
    if (deviceId !== 'default') {
        const preferences = _.get(globals.data.cache.userPreferences, deviceId);
...
function pushMatchToGlobalMatches(globals, matchInfo) {
    ...
    const existingMatchIndex = globals.data.matches.findIndex(match => match.id === matchInfo.id);
    const existingMatch = globals.data.matches[existingMatchIndex];
    ...
    if (existingMatch) {
        ...
        if (isMatchToday) {
            ...
            // Otherwise, delete the existing match and create a new proxy object
                ...
                // Remove the match with the same ID from the globals.data.matches array
                globals.data.matches.splice(existingMatchIndex, 1);
...
async function getFixtures(globals, deviceId, limit, tvOnly) {
    ...
    return await getMatchesProcessed(globals, fixtures, deviceId, 'asc', limit, tvOnly);
}

// Returns an array of results, i.e. matches that have finished and have a date either today or in the past
async function getResults(globals, deviceId, limit, tvOnly) {
    let matches = globals.data.matches;
    let results = [];
    for (const match of matches) {
    ...
    return await getMatchesProcessed(globals, results, deviceId, 'desc', limit, tvOnly);
}
...
async function getMatchesProcessed(globals, matches, deviceId, dateOrder, limit = 0, tvOnly = false) {

    let processedMatches = [];

    for (const match of matches) {
        if ((deviceId && match.interestedUsers && match.interestedUsers.includes(deviceId)) && (!tvOnly || _.get(match, 'tvInfo__channelInfo__fullName')))
            processedMatches.push(utils.getJsonExpanded(match));
    }
    return getMatchesSorted(processedMatches, dateOrder, limit);
}
...
function getMatchesSorted(matches, dateOrder, limit) {
    const sortedMatches = _.orderBy(matches, ['dateTimeUtc', 'kickOffTime', 'competition.weight', 'competition.name', 'competition.subHeading', 'homeTeam.names.displayName', 'tvInfo.channelInfo.fullName'], [dateOrder, dateOrder, 'desc', 'desc', 'asc', 'desc', 'asc']);
    if (limit > 0)
