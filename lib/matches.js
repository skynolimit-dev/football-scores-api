...
async function setMatchesForDate(globals, date, useSimulator) {
    ...
    console.info('Getting matches from:', url);
        ...
        (match, path) => {
            ...
            if (path && path.length > 0) {
                if (path.includes('homeTeam__score') || path.includes('awayTeam__score') || path.includes('timeLabel') || path.includes('time') || path.includes('started') || path.includes('finished')) {
                    console.log('Match updated:', path, match.id, match.homeTeam__names__displayName, match.awayTeam__names__displayName, match.homeTeam__score, match.awayTeam__score, match.started, match.finished, match.time, match.timeLabel);
