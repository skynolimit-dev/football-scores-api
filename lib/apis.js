...
function serve(globals, matches, news, app) {
    ...
    app.get('/api/v1/user/:deviceId/matches/fixtures', async (req, res) => {
        res.json(await matches.getFixtures(globals, req.params.deviceId, req.query.limit, req.query.tvOnly));
    });

    // API endpoint to get default fixtures (used by the widget)
    app.get('/api/v1/matches/fixtures', async (req, res) => {
        res.json(await matches.getFixtures(globals, 'default', req.query.limit, req.query.tvOnly));
    });
    ...
    app.get('/api/v1/user/:deviceId/matches/results', async (req, res) => {
        res.json(await matches.getResults(globals, req.params.deviceId, req.query.limit, req.query.tvOnly));
    });

    // API endpoint to get default results (used by the widget)
    app.get('/api/v1/matches/results', async (req, res) => {
        res.json(await matches.getResults(globals, 'default', req.query.limit, req.query.tvOnly));
    });
    ...
    // API endpoint to get the widget source for the given user and widget category
    app.get('/api/v1/user/:deviceId/widget/:widgetCategory/:tvOnly', async (req, res) => {
        const widgetSource = await widget.getWidget(globals, req.params.deviceId, req.params.widgetCategory, req.params.tvOnly);
        // Return the widget source as a textfile download
        res.setHeader('Content-Type', 'text/javascript');
        res.setHeader('Content-Disposition', `attachment; filename=TopScores_${req.params.widgetCategory}.scriptable`);
