...
function serve(globals, matches, news, app, predictor) {
    ...
    // API endpoint to get all Predictor matches - for debugging only
    app.get('/api/v1/debug/matches/predictor', async (req, res) => {
        res.json(globals.data.predictorMatches);
    });

    // API endpoint to get raw Predictor matches for the given user (device) ID - for debugging only
    app.get('/api/v1/debug/matches/predictor/user/:deviceId/raw', async (req, res) => {
        res.json(globals.data.predictorMatches[(globals, req.params.deviceId)]);
    });
    ...
    // API endpoint to get the user preferences cache - for debugging only
    app.get('/api/v1/debug/user/preferences/cache', (req, res) => {
        res.json(globals.data.userPreferencesCache);
    });
    ...
    app.get('/api/v1/matches/fixtures', async (req, res) => {
        res.json(await matches.getFixtures(globals, null, req.query.limit, true));
    });
    ...
    app.get('/api/v1/matches/results', async (req, res) => {
        res.json(await matches.getResults(globals, null, req.query.limit, true));
    });

    // API endpoint to get Predictor matches for the given user (device) ID
    app.get('/api/v1/user/:deviceId/matches/predictor', async (req, res) => {
        ...
        // TODO: Remove
        // res.json(await matches.getPredictorMatches(globals, req.params.deviceId));
    });

    // API endpoint to start the given simulated match for the given user (device) ID
    app.post('/api/v1/matches/predictor/user/:deviceId/match/:matchId/start', async (req, res) => {
        ...
        res.json(predictor.initMatch(globals, predictorDetails));
    });

    // API endpoint to pause the given simulated match for the given user (device) ID
    app.post('/api/v1/matches/predictor/user/:deviceId/match/:matchId/pause', async (req, res) => {
        const predictorDetails = {
            matchId: req.params.matchId,
            device: {
                id: req.params.deviceId
        ...
        res.json(predictor.pauseMatch(globals, predictorDetails));
    });

    // API endpoint to resume the given simulated match for the given user (device) ID
    app.post('/api/v1/matches/predictor/user/:deviceId/match/:matchId/resume', async (req, res) => {
        const predictorDetails = {
            matchId: req.params.matchId,
            device: {
                id: req.params.deviceId
        ...
        res.json(predictor.resumeMatch(globals, predictorDetails));
    });
    ...
    app.put('/api/v1/user/:deviceId/userDeviceInfo', async (req, res) => {
        res.json({success: await users.set(req.body, req.params.deviceId)});
    });
    ...
    app.put('/api/v1/user/:deviceId/preferences', async (req, res) => {
        res.json({success: await users.setPreferences(req.body, req.params.deviceId)});
    });
